...
size_t BufferedAsyncSerial::readUntil(uint8_t* data, size_t len, const char delim='\n') {

    lock_guard<mutex> l(readQueueMutex);
    auto it = std::find(readQueue.begin(), readQueue.end(), delim);
    if( it == readQueue.end() ) return 0; // not found
    size_t result = std::distance(readQueue.begin(), it);
    // Handle a partial packet
    if( result < len ) {
        it = std::find(it+1, readQueue.end(), delim);
    }
    if( it == readQueue.end() ) return 0;
    std::copy(readQueue.begin(), it, data);
    result = std::distance(readQueue.begin(), it);
    it += 1; // remove the delimiter from the queue
    readQueue.erase(readQueue.begin(), it);
    countDataAvailable = readQueue.size();
    return result;
...
void BufferedAsyncSerial::readCallback(const char *data, size_t len)
{
    ...
    // lock_guard<mutex> l_f(newDataAvailableMutex);
    countDataAvailable += len;
}
