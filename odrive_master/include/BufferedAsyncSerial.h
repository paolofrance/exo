...
public:
    ...
    */
    ...
    /**
     * Read some data until a stop char asynchronously. Returns immediately.
     * \param data array of uint8_t to be read through the serial device
     * \param delim stop byte 
     * \return numbr of character actually read (0 if delim has not been received yet)
     */
    size_t readUntil(uint8_t* data, size_t len, const char delim);
    ...
    volatile ssize_t countDataAvailable = 0;
...
private:
    ...
    std::mutex readQueueMutex, newDataAvailableMutex;
};
