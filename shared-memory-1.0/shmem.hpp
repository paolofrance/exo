...
public:
    ...
    /// @param fname 
    shared_memory(const std::string& fname) : fname(fname) 
    {
    }
    ...
    /**
     * @brief Dump the entire content of the shared memory object formatted as 4-byte words
     * 
     */
    void dump() const {
        dump(BLOCK_SIZE/4, 4);
    ...
    /**
     * @brief Dump the content of the shared memory object specifying the number of elements to print and their width 
     * 
     * @param num_elements number of elements to print to console
     * @param width number of bytes for each element (default: sizeof(float) = 4)
     */
    void dump(size_t num_elements, size_t width=sizeof(float)) const {

        std::cout << "\nMemory @ " << (uintptr_t) p_mem;
        for( size_t i = 0; i < width*num_elements; i++ ) {
            if( i % width == 0 ) std::cout << "\n0x" << std::setw(2) << std::setfill('0') << std::hex << i << " ";
            std::cout << std::setw(2) << std::setfill('0') << std::hex << int(p_mem[i]) << " ";
        }
        std::cout << "\n\n";
